...
def calculate_average_marginal_impact(df, model):
    """
    Calculate the Average Marginal Impact (AMI) of each strategy based on the fitted regression model.
    AMI estimates the additional sales generated by each strategy, assuming no other strategies are applied.
    """
    ...
    ami_data = {
        'Strategy': ['Strategy 1', 'Strategy 2', 'Strategy 3'],
        'Average Marginal Impact (AMI)': [AMI_strategy1, AMI_strategy2, AMI_strategy3]
    }
    
    ami_df = pd.DataFrame(ami_data)

    # Display the AMI in a table
    st.subheader("Average Marginal Impact of Each Strategy")
    st.table(ami_df)
